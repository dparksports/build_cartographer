...
make -j8
    ...
    tips: we compile static library with --disable-shared CXXFLAGS = "-fPIC". When compiling dynamic library dll / so, if you rely on static library (such as profobuf), then static library needs to add -fPIC, otherwise Dynamic library compilation error.
    For CMake, use cmake CMAKE_CXX_FLGAS = "-fPIC" ..
